...
- split: тот же честный сценарий, который не нарушает уже принятые правила проекта.
...
- обучение `ALS` или близкой учебной factorization-модели;
...
- минимальный модуль advanced-модели;
- явный интерфейс `fit` / `recommend`;
- аккуратная работа с ID mapping;
- отсутствие претензии на industrial-scale реализацию.
